...
from .utils import get_confidence, get_entropy, ind_to_onehot
...
class CMIEstimator(pl.LightningModule):
    ...
    _train_outs_l: list[dict[str, torch.Tensor]]
    _val_pred_l: list[list[torch.Tensor]]
    _val_y_l: list[torch.Tensor]
    ...
    def __init__(
        ...
        if cmi_scaling not in ("none", "positive", "bounded"):
            raise ValueError(
                'cmi_scaling must be one of "none", "positive", or "bounded"'
            )
        ...
        # book keeping for train and validation steps
        self._train_outs_l = list()
        self._val_pred_l = list()
        self._val_y_l = list()
    ...
    def training_step(self, batch, batch_idx):
        ...
        for _ in range(self.max_features):
            ...
            random = torch.tensor(
                np.random.choice(self.mask_size, size=len(x)), device=x.device
            )
        ...
        self._train_outs_l.append(out)
        return out

    # def training_epoch_end(self, outputs):
    #     # Get mean train losses.
    #     pred_loss = torch.stack([out["predictor_loss"] for out in outputs]).mean()
    #     value_network_loss = torch.stack(
    #         [out["value_network_loss"] for out in outputs]
    #     ).mean()
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # return pred, y
        self._val_pred_l.append(pred_list)
        self._val_y_l.append(y)
        return pred_list, y
    ...
    def on_validation_epoch_end(self) -> None:
        pred_list = self._val_pred_l
        y_list = self._val_y_l
        y = torch.cat(y_list)
        preds_cat = [torch.cat(preds) for preds in zip(*pred_list)]
        pred_loss_ = [self.loss_fn(preds, y).mean() for preds in preds_cat]
        val_loss_ = [self.val_loss_fn(preds, y) for preds in preds_cat]
    ...
    def on_predict_start(self):
        if not hasattr(self, "mode"):
            print(
                "Must specify stopping criterion. Recommended usage is via `inference` function"
            )
    ...
    def predict_step(self, batch, batch_idx):
        ...
        for step in range(self.mask_size):
            ...
            if self.mode == "penalized":
                ...
                accept_sample = (
                    torch.max(pred_cmi / self.feature_costs, dim=1).values > self.lam
                )
                ...
                accept_sample = (
                    torch.sum(features_selected * self.feature_costs, dim=1)
                    <= self.budget
                )
            ...
            mask[accept_sample] = torch.max(
                mask[accept_sample], selection[accept_sample]
            )
