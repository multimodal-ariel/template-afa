...
class VAEAC(Module):
    ...
    def make_latent_distributions(self, batch, mask, no_proposal=False):
        ...
        """
        observed = mask_batch(batch, mask)
        if no_proposal:
            proposal = None
        else:
            full_info = torch.cat([observed, mask], 1)
    ...
    def generate_samples(self, batch, mask, K=1):
        """
        ...
        for i in range(K):
            ...
            sample_param = self.generative_network(latent)
            sample = self.rec_log_prob.sampler(sample_param)
