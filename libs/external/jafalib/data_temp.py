...
def data_split_n_ready(
    ...
    split = np.floor(val_test_split * dataset_size).astype(np.int32)
    ...
    return (
        pick(train_indices, True),
        pick(val_indices, False),
        pick(test_indices, False),
    )


def data_load(
    data_type="cube_20_0.3",  # "cube_Nfeatures_sigma" or csv
    random_seed=123,
    ...
    if data_type == "csv":
        ...
        features, labels = gen_cube(
            n_features, data_points=n_datapoints, sigma=sigma, seed=random_seed
        )
        exist = None
        action2features = None
        cost = None
...
class DataTemp:
    def __init__(
        ...
        self.n_actions = (
            len(action2features) + 1
            if action2features is not None
    ...
    def next_batch(self, batch_size):
        ...
        if self.index + batch_size <= self.n_data:
            ...
            exist = (
                self.exist[self.index : self.index + batch_size]
            ...
            if self.iter:
                ...
                labels = np.concatenate((labels, self.labels[:new_index]), axis=0)
                exist = (
                    np.concatenate((exist, self.exist[:new_index]), axis=0)
                    if self.exist is not None
                    else None
                )
...
fl_keys = list(
    sorted(
        fatty_liver_3_cost.keys(),
        key=lambda x: x[0] if isinstance(x[0], int) else x[0][0],
    )
)

fl_action2features = {
    i: x[0] if isinstance(x[0], int) else (lambda x: [y[0] for y in x])(x)
    for i, x in enumerate(fl_keys)
}
...
imt_cost = {
    ((0, "HCC_1"), (1, "HCC_2"), (2, "HCC_3"), (3, "HCC_4"), (4, "HCC_5")): 100,
    (5, "Sex"): 1,
    (6, "Age"): 1,
    (
        (7, "Stiffness (kPa)"),
    ...
    (
        ...
        (28, "SKELETAL_MUSCLE"),
        (29, "FAT_RATE"),
        (30, "ABDOMINAL_FAT_RATE"),
        (31, "VISCERAL_FAT"),
    ): 100,
    (32, "Testosterone"): 12100,
    (33, "T3"): 10800,
    (34, "TPO-ab"): 1e5,
    (35, "Vit D"): 1e5,
    (36, "FT4"): 12100,
...
imt_keys = list(
    sorted(imt_cost.keys(), key=lambda x: x[0] if isinstance(x[0], int) else x[0][0])
)

imt_action2features = {
    i: x[0] if isinstance(x[0], int) else (lambda x: [y[0] for y in x])(x)
    for i, x in enumerate(imt_keys)
}
