...
def weight_xavier_init(m):
    if type(m) == th.nn.Linear:
        th.nn.init.xavier_normal(m.weight)
    elif type(m) == th.nn.LSTMCell:
        th.nn.init.xavier_normal(m.weight_ih)
        th.nn.init.xavier_normal(m.weight_hh)
...
class MLP(th.nn.Module):
    def __init__(
        ...
        for hidden_size in hidden_sizes:
            self.layers.append(th.nn.Linear(in_size, hidden_size, bias=bias))
...
class SetEncoder(th.nn.Module):
    def __init__(
        ...
        self.lstm = th.nn.LSTMCell(embedded_dim, lstm_size)
        # self.module_list = th.nn.ModuleList([self.embedder, self.lstm])
        ...
        if not simple:
            ...
            self.attention = th.nn.ModuleList(
                [
                    th.nn.Linear(lstm_size, proj_dim, bias=False),
                    th.nn.Linear(embedded_dim, proj_dim, bias=True),
                    th.nn.Linear(proj_dim, 1, bias=True),
                ]
            )
        elif embedded_dim != lstm_size:
            self.attention = th.nn.Linear(lstm_size, embedded_dim, bias=False)
            # th.nn.init.xavier_normal(self.attention.weight)
        ...
        def _compute_attention_sum(q, m, length):
            ...
            assert th.max(length) == m.size()[1]
            max_len = m.size()[1]
            if simple:
                if q.size()[-1] != m.size()[-1]:
                    q = self.attention(q)  # batch_size x embedded_dim
                weight_logit = th.bmm(m, q.unsqueeze(-1)).squeeze(
                    2
                ...
                packed = pack(
                    th.nn.functional.relu(proj_m + proj_q),
                    list(length),
                    batch_first=True,
                )
    ...
    def forward(self, state, length):
        ...
        nonzero = th.sum(length > 0).cpu().numpy()  # encode only nonzero points
        if nonzero == 0:
        ...
        if self.normalize:
            embedded = th.nn.functional.normalize(embedded, dim=1)
...
class DuelingNet(th.nn.Module):
    ...
    def forward(self, encoded):
        ...
        tmp = th.nn.functional.relu(tmp)
        self.adv = self.pi_net(tmp)  # batch_size x n_actions
        self.v = self.v_net(tmp)  # batch_size x 1

        output = self.v + (self.adv - th.mean(self.adv, dim=1, keepdim=True))

        return output  # th.cat((self.pi, self.v), 1)


class DFSNet(th.nn.Module):
    encoder: Union[SetEncoder, MLP]
    classifier: MLP
    policy: DuelingNet

    def __init__(self, encoder, classifier, policy):
        ...
        self.policy = policy
        self.n_features = (
            classifier.n_features if encoder is None else encoder.n_features
        )
        ...
        self.clf_weight_params = [
            clf_params[key] for key in clf_params.keys() if "weight" in key
        ]
        self.clf_bias_params = [
            clf_params[key] for key in clf_params.keys() if "weight" not in key
        ]

    def forward(self, inputs, length):
        sorted_, indices = th.sort(length, -1, descending=True)
        _, invert = th.sort(indices)
...
#     batch_in = th.zeros((batch_size, n_features, n_features + 1))
#     a = np.arange(n_features * (n_features + 1) * 1.0).reshape(
#         n_features, n_features + 1
#     )
#     vec_1 = th.FloatTensor(a)  # 3 x 2
#     a[2] = 0
#     vec_2 = th.FloatTensor(a)
#     a[1] = 0
#     vec_3 = th.FloatTensor(a)
#     a[0] = 0
#     vec_4 = th.FloatTensor(a)
...
#     batch_in[0] = vec_4
#     seq_length = [0, 2, 3, 1]
#     seq_length = th.LongTensor(seq_length)
...
#     in_size = input_size
#     for hidden_size in hidden_sizes:
#         layers.append(th.nn.Linear(in_size, hidden_size, bias=bias))
#         layers.append(th.nn.ReLU())
#         in_size = hidden_size
#     layers.append(th.nn.Linear(in_size, output_size, bias=bias))
#     model = th.nn.Sequential(*layers)
#     return model
# """
