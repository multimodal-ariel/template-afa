...
def binary_cross_entropy(logits, labels):
    ...
    loss = (
        ...
        - logits * labels
        + max_val
        + np.log(np.exp(-max_val) + np.exp(-logits - max_val))
    )
    p = sigmoid(logits)
...
class Env(object):
    def __init__(self, args, n_envs, r_cost, data, classify_fn):
        ...
        self.batch_acquired = np.zeros((self.n_envs, self.n_features)).astype(np.uint8)
        self.batch_obs = np.zeros(
            (self.n_envs, self.n_features, self.n_features + 1), dtype=np.float32
        ).astype(np.float32)
        self.batch_returns = np.zeros(self.n_envs, dtype=np.float32)
        self.rewards = np.zeros(self.n_envs, dtype=np.float32)
        self.q_a = np.zeros((self.n_envs, self.n_actions), dtype=np.float32)

        # TODO : gpu.. .cuda()
        self.var_inputs = th.from_numpy(self.batch_inputs)
        self.var_labels = th.from_numpy(self.batch_labels)
        self.var_exist = th.from_numpy(self.batch_exist)
        self.var_acquired = th.from_numpy(self.batch_acquired)
        self.var_obs = th.from_numpy(self.batch_obs)
        self.var_rewards = th.from_numpy(self.rewards)

        if self.n_actions != self.n_features + 1:
            self.batch_acquired_aux = np.zeros_like(self.batch_exist)
            self.var_acquired_aux = th.from_numpy(self.batch_acquired_aux)

    def calc_reward(self, p_y_logit, labels):
        if self.n_classes > 2:
            crss_ent, prob = cross_entropy(p_y_logit, labels)
        else:
            crss_ent, prob = binary_cross_entropy(p_y_logit.reshape(-1), labels)
    ...
    def get_current_batch_with_random_features(self):
        sampled = np.random.binomial(
            1, 0.5, self.batch_exist.shape
        ) * self.batch_exist.astype(int)
        batch_obs = np.zeros(
            (self.n_envs, self.n_features, self.n_features + 1), dtype=np.float32
        ).astype(np.float32)
        if self.n_actions == self.n_features + 1:
        ...
        return th.from_numpy(batch_obs), self.var_labels, th.from_numpy(length)
    ...
    def _nonterminal_step(self, actions, nonterminal, length):
        if self.n_actions == self.n_features + 1:
            ...
            self.batch_obs[nonterminal, length[nonterminal], 0] = self.batch_inputs[
                nonterminal, actions[nonterminal]
            ]
            self.batch_obs[
                nonterminal, length[nonterminal], actions[nonterminal] + 1
            ] = 1
        else:
            assert np.all(
                self.batch_acquired_aux[nonterminal, actions[nonterminal]] == 0
            )
            self.batch_acquired_aux[nonterminal, actions[nonterminal]] = 1
            x = np.where(nonterminal)[0]
            if nonterminal.any():
                x_ = np.concatenate(
                    [[i] * np.array(self.action2features[actions[i]]).size for i in x]
                )
                y_ = np.concatenate(
                    [np.array(self.action2features[actions[i]]).reshape(-1) for i in x]
                )
                self.batch_acquired[x_, y_] = 1
                len_acq = [np.array(self.action2features[actions[i]]).size for i in x]
                tmp1 = np.concatenate(
                    [
                        [length[i]] * np.array(self.action2features[actions[i]]).size
                        for i in x
                    ]
                )
    ...
    def step(self, actions):
        ...
        if not np.all(nonterminal):
            p_y_logit = (
                self.classify(
                    th.from_numpy(self.batch_obs[~nonterminal]),
                    th.from_numpy(self.batch_acquired[~nonterminal]),
                )
                .detach()
                .cpu()
                .numpy()
            ...
            reward, prob = self.calc_reward(p_y_logit, self.batch_labels[~nonterminal])
        ...
        var_dones = th.from_numpy((~nonterminal).astype(np.uint8))
    ...
    def test_step(self, actions, q_a=None):
        ...
        if not np.any(terminal_):
            ...
            p_y_logit = (
                self.classify(
                    th.from_numpy(self.batch_obs[notignore][terminal_]),
                    th.from_numpy(self.batch_acquired[notignore][terminal_]),
                )
                .detach()
                .cpu()
                .numpy()
            )
            reward, prob = self.calc_reward(
                p_y_logit, self.batch_labels[notignore][terminal_]
            )
            ...
            if self.n_classes > 2:
                correct = np.equal(
                    np.argmax(prob, 1), self.batch_labels[notignore][terminal_]
                )
                probs = prob[np.arange(len(np.where(terminal_)[0])), labels]
            else:
                sigmoid_ = prob  # sigmoid(p_y_logit.reshape(-1))
                correct = np.equal(prob >= 0.5, self.batch_labels[notignore][terminal_])
                probs = np.where(
                    self.batch_labels[notignore][terminal_], prob, 1 - prob
                )
