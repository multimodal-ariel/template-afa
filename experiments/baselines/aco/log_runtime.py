...
def aaco_runtime(aaco_run_p: str, n_instances: int, plf: pl.Fabric):
    @dataclass
    class AACOMainConf:
        data: Any
        init_fidx: int
    ...
    metrics_d = dict()
    metrics_d["feature observed"] = th.mean(
        th.sum(
...
def main(cfg: MainConf):
    ...
    aaco_runtime(aaco_run_p=run_p, n_instances=cfg.n_instances, plf=plf)
