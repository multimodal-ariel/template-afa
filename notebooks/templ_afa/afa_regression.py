...
class SoftmaxSelector(th.nn.Module):
    ...
    def __init__(
        self, n_covs: int, allfcombs_l: list[tuple[int, ...]], nnet: th.nn.Module
    ) -> None:
        super().__init__()
        self.n_covs = n_covs
        self.n_templates = len(allfcombs_l)
        self.nnet = nnet
        self._allxacts = th.zeros((self.n_templates, self.n_covs), dtype=th.float32)
        for _i, _fc in enumerate(allfcombs_l):
            self._allxacts[_i, _fc] = 1

    def forward(self, sinps: th.Tensor) -> th.Tensor:
...
def _fit_iter(
    ...
    n_templates: int = selector.n_templates
    allxacts: th.Tensor = selector._allxacts
    opt: th.optim.Optimizer = tstate["opt"]
    slosses_l: list[th.Tensor] = list()
    for bxps in tloader:
        bxps = bxps.to(device=plf.device)
        bsinps: th.Tensor = bxps["sinps"]
        bstargs: th.Tensor = bxps["rewards"].to(device=plf.device)
        # (bsz, n_templates, n_covs)
        bxacts: th.Tensor = bxps["xacts"][:, None, :].expand(-1, n_templates, -1)
        ballxacts: th.Tensor = allxacts[None, :, :].expand(len(bxps), -1, -1)
        # (bsz, n_templates)
        bsmouts: th.Tensor = th.all(th.isclose(bxacts, ballxacts), dim=2)
        bsmouts = bsmouts.to(device=plf.device)
        bsouts_: th.Tensor = selector(bsinps)
        # (bsz, )
        bsouts: th.Tensor = th.sum(bsouts_ * bsmouts, dim=1)
