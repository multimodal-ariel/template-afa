...
class DecisionTreeConverter:
    def __init__(self):
        ...
        self.node_info = {}  # node_id -> {feature, threshold, samples, classification, etc}
        self.color_mapping = {
    ...
    def create_intervals(self, feature, thresholds):
        ...
        for i in range(1, len(thresholds)):
            intervals.append((f'({thresholds[i-1]}, {thresholds[i]}]', thresholds[i-1], thresholds[i]))
    ...
    def generate_nary_dot(self, output_filename):
        ...
        with open(output_filename, 'w') as f:
            ...
            # Root node
            f.write(f'    root [label="{root_feature}", style=filled, fillcolor=lightgray, fontsize=14, fontweight=bold];\n\n')
            ...
            for i, (interval_str, lower, upper) in enumerate(root_intervals):
                ...
                for path in paths:
                    root_condition = path['conditions'][0] if path['conditions'] else None
    ...
    def process_sub_features(self, f, paths, parent_node, level):
        ...
        if len(set(classifications)) == 1:
            ...
            f.write(f'    {leaf_node} [label="{classification}", style=filled, fillcolor={color}')
            if fontcolor == 'white':
        ...
        f.write(f'    {feature_node} [label="{best_feature}", style=filled, fillcolor=lightgray, fontweight=bold];\n')
        ...
        for j, (interval_str, lower, upper) in enumerate(intervals):
            ...
            for path in feature_paths:
                if len(path['conditions']) > level:
                    ...
                    if self.condition_matches_interval(condition[1], condition[2], lower, upper):
                        matching_paths.append(path)
