...
class GlobalLogicExtractor:
    def __init__(self, model_dir, tmpls_path, feature_names=None):
        ...
        self.tmpls = torch.load(tmpls_path, map_location='cpu')
        if isinstance(self.tmpls, list):
            self.tmpls = torch.stack(self.tmpls)
        ...
        print(f"[TAFA-Interp] Loaded {len(self.models)} stage trees and {len(self.tmpls)} templates.")
    ...
    def extract(self, init_feature_idx):
        if init_feature_idx >= self.n_features:
            print(f"[Warning] Init feature index {init_feature_idx} >= n_features {self.n_features}. Defaulting to 0.")
            init_feature_idx = 0

        print(f"[TAFA-Interp] Starting extraction with initial feature: {self.feature_names[init_feature_idx]}")
        ...
        print(f"[TAFA-Interp] Extraction complete. Found {len(self.extracted_paths)} logical paths.")
    ...
    def _trace_single_tree(self, tree, node_idx, current_mask, current_rules):
        if tree.children_left[node_idx] == _tree.TREE_LEAF:
            ...
            if class_idx >= len(self.tmpls): return # Safety
        ...
        if feature_idx >= self.n_features:
            ...
            if mask_val <= threshold:
                self._trace_single_tree(tree, tree.children_left[node_idx], current_mask, current_rules)
            else:
                self._trace_single_tree(tree, tree.children_right[node_idx], current_mask, current_rules)
            return
        ...
        if is_observed:
            rule_left = {'idx': feature_idx, 'name': feature_name, 'op': '<=', 'val': threshold}
            self._trace_single_tree(tree, tree.children_left[node_idx], current_mask, current_rules + [rule_left])
            
            rule_right = {'idx': feature_idx, 'name': feature_name, 'op': '>', 'val': threshold}
            self._trace_single_tree(tree, tree.children_right[node_idx], current_mask, current_rules + [rule_right])
    ...
    def validate_with_data(self, X_val):
        ...
        print(f"[TAFA-Interp] Validation coverage: {matched_count}/{len(X_val)} ({(matched_count/len(X_val)*100):.1f}%) matches.")
    ...
    def verify_consistency(self, X_val, init_feature_idx):
        ...
        print(f"  Same # Features:  {correct_features}/{valid_samples} ({(correct_features/valid_samples*100):.1f}%)")
        print(f"  Exact Set Match:  {correct_set}/{valid_samples} ({(correct_set/valid_samples*100):.1f}%)")
    ...
    def evaluate_static_accuracy(self, X_val, y_val, classifier, device='cpu'):
        ...
        M_tensor = torch.tensor(masks, dtype=torch.float32).to(device)
        ...
        while True:
            ...
            for i in range(n):
                ...
                for j in range(i + 1, n):
                    ...
                    all_feats = set(path_a['intervals'].keys()) | set(path_b['intervals'].keys())
                    diff_feats = []
                    
                    for f in all_feats:
                        range_a = path_a['intervals'].get(f, [-float('inf'), float('inf')])
                        range_b = path_b['intervals'].get(f, [-float('inf'), float('inf')])
                        
                        if not (np.isclose(range_a[0], range_b[0], atol=1e-6) and 
                                np.isclose(range_a[1], range_b[1], atol=1e-6)):
                            diff_feats.append(f)
                    
                    if len(diff_feats) == 1:
                        f_diff = diff_feats[0]
                        range_a = path_a['intervals'].get(f_diff, [-float('inf'), float('inf')])
                        range_b = path_b['intervals'].get(f_diff, [-float('inf'), float('inf')])
    ...
    def generate_report(self):
        ...
        sorted_outcomes = sorted(grouped.items(), key=lambda x: sum(p['count'] for p in x[1]), reverse=True)
        ...
        for outcome, raw_paths in sorted_outcomes:
            ...
            for i, path in enumerate(agg_paths):
                ...
                if not intervals:
                    ...
                    rule_str = " AND ".join(rule_strs) if rule_strs else "DEFAULT (Always True)"
