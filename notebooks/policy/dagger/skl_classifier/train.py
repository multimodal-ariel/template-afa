...
for _itr in tqdm.trange(iterations):
    # TRAINING
    print("*** Iteration", _itr + 1, "/", iterations, "***")
    print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time())))
    metrics_func.reset()
    ...
    for _cardinality in buffer_state.keys():
        ...
        _single_tree = skl_tree.DecisionTreeClassifier(
        ...
        _single_tree.fit(states, actions)  # , sample_weight=weights

        _single_student = SKLClassifierPolicy(_single_tree, tmpls.shape[1])
