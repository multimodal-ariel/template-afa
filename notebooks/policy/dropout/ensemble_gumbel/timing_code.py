...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate differentiable template policy."
    )
    parser.add_argument(
        "--dataset",
        "-d",
        type=str,
        default="mnist",
        help="Dataset name (mnist, big5, grid, gas, cube).",
    )
    parser.add_argument("--gpu", type=int, default=0, help="GPU index to use (0-3).")
    parser.add_argument(
        "--model_path", type=str, default=None, help="Path to trained policy checkpoint"
    )
...
if _dataset_name == "mnist":
    ...
    mktmpl_run_dir = (
        f"experiments/make_template/outputs/fashion_cnnet/20251120_232234/0"
    )
elif _dataset_name == "fashion":
    mktmpl_run_dir = (
        f"experiments/make_template/outputs/fashion-16x16_cnnet/20260102_202634/0"
    )
...
# load data
_tdata_shuffle_idxs = torch.load(
...
print(
    f"*** Starting Evaluation: {_dataset_name} | Lambda: {alpha} | Device: {device}",
    flush=True,
)
...
def evaluate_policy(
    policy, x, y, start_dim, alpha, batch_size=1024, max_steps_eval=None
):
    """
    Eval
    """
...
surrogate = SurrogateClassifier(
    n_covs, n_labels, hidden_dim=hidden_size, dataset_name=args.dataset
).to(device)
