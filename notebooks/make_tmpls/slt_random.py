...
def make_templates(
    ...
    while True:
        ...
        if _fc not in tmpl_fcs and _fc not in tmpl_fcs:
            tmpl_fcs.append(tuple(_fc))
    tmpls: th.Tensor = th.zeros((n_tmpls, n_covs), dtype=th.long)
    for _j, _fc in enumerate(tmpl_fcs):
        tmpls[_j, _fc] = 1
    return tmpls
...
output_dir: str = os.path.join("outputs", "run", data_name, "slt_random")
