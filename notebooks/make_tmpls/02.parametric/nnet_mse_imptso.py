...
def nnet_cost_est(
    ...
    mus: th.Tensor = mu[None, :] * (th.ones_like(fms) - fms)
    inps_: th.Tensor = th.where(fms == 1, inps[:, :n_covs], mus)
    # TODO might have to switch to looping over tmpl dim
    # (n, n_tmpls, n_covs)
    tmpls_: th.Tensor = tmpls[None, :, :].expand(len(inps), -1, -1)
    # (n, n_tmpls, 2 * n_covs)
    inps_ = inps_[:, None, :].expand(-1, len(tmpls), -1)
    # (n * n_tmpls, 1)
...
def _make_fit_bsinps(
    bstdata: thd.TensorDict, mu: th.Tensor, init_fidx: int, tmpls: th.Tensor
    ...
    mus: th.Tensor = mu[None, :] * (th.ones_like(bfms) - bfms)
    n_covs: int = bxs.shape[1]
    # randomly drop features
    bnms: th.Tensor = th.randint(0, 2, (bsz, n_covs), device=device)
    bnms = th.clamp(bnms - bfms, 0.0, 1.0)
    bnms[:, init_fidx] = 1
    # (bsz, 2 * n_covs)
    bsinps: th.Tensor = th.cat((th.where(bnms == 1, bxs, mus), bfms), dim=1)
    # (bsz, 1)
    bstargs: th.Tensor = th.gather(
        bstdata["cels"], dim=1, index=btmplidxs[:, None].to(device=device)
    )
...
def _fit_iter(
    ...
    for bstdata in tloader:
        ...
        bsinps, bstargs = _make_fit_bsinps(
            bstdata=bstdata, mu=mu, init_fidx=init_fidx, tmpls=tmpls
        )
        # (bsz, 1)
...
# %%
mu: th.Tensor = th.mean(tdata["xs"], dim=0)
...
for _data in vdata:
    _pyhat, _fobsd_l, _fcomb = _tmplfns.run_one_episode_all_obsd(
        ...
        cost_est=lambda x: nnet_cost_est(
            x, mu=mu, nnet=nnet, lmbda=lmbda, tmpls=tmpls, device=plf_nnet.device
        ),
        init_fidx=init_fidx,
        tmpls=tmpls,
        plf=plf_tmpl,
