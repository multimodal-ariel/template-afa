...
def _fill_fcs_set_with_random_tmpls_with_tracking(
    ...
    for _k, (_count, _fcs_set) in enumerate(
        ...
        for _ in range(5 * _count):
            if len(_fcs_set) - _init_fcs_set_len >= _count:
                break
            _ps: th.Tensor = (
...
def make_templates_fix_rounds_with_tracking(
    ...
    for _i in tqdm.trange(
        ...
        if (_i % eval_every_n_iter) == 0 or _i + 1 == n_rounds:
...
to_update_classifier: bool = True
bsz: int = 128000

# %%
# configure logger and ckpt path
output_dir: str = os.path.join("outputs", "run", data_name, "mutate")
