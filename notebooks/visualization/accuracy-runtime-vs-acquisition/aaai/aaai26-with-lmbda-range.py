...
def load_baseline_metrics(exp_p: str):
    """Modified to include lambda tracking for ACO method"""
    ...
    for run_p in sorted(
        ...
        config_p: str = os.path.join(run_p, ".hydra", "config.yaml")

        if not os.path.exists(metrics_p):
            lgr.warning(f"{metrics_p} does not exist")
            continue

        # Load config to get lambda value
        lmbda_val = None
        if os.path.exists(config_p):
            try:
                run_cfg = OmegaConf.load(config_p)
                lmbda_val = getattr(run_cfg, "lmbda", None)
            except Exception as e:
                lgr.warning(f"Could not load config from {config_p}: {e}")

        metrics_df: pd.DataFrame = (
            pd.read_csv(metrics_p).groupby("step").sum(min_count=1)
        )

        # Add lambda column to dataframe
...
def track_visible_lambda_ranges(pltcfg, metrics_dl):
    """Track lambda ranges that are visible in the plots per dataset"""
    visible_lambda_ranges = {}  # {method: {dataset: {'min': x, 'max': y}}}

    for _expcfg, _metrics_d in zip(pltcfg.expcfgs, metrics_dl):
        dataset_name = _expcfg.subtitle  # Use subtitle as dataset identifier

        # Get plot limits
        _xlim = [_expcfg.xlim[0] or 0, _expcfg.xlim[1] or float("inf")]
        _ylim = [_expcfg.ylim_row0[0] or 0, _expcfg.ylim_row0[1] or 1]

        for _name, _metrics_dfl in _metrics_d.items():
            if _name not in ["aco", "tafa"] or len(_metrics_dfl) == 0:
                continue

            if _name not in visible_lambda_ranges:
                visible_lambda_ranges[_name] = {}
            if dataset_name not in visible_lambda_ranges[_name]:
                visible_lambda_ranges[_name][dataset_name] = {
                    "min": float("inf"),
                    "max": float("-inf"),
                }

            for _mdf in _metrics_dfl:
                if "lmbda" not in _mdf.columns:
                    continue

                # Determine prefix for this dataframe
                _prefix = (
                    "eval"
                    if "eval/acc" in _mdf
                    else "eval_val" if "eval_val/acc" in _mdf else "eval_dagger"
                )
                if (
                    f"{_prefix}/feature observed" not in _mdf
                    or f"{_prefix}/{pltcfg.key}" not in _mdf
                ):
                    continue
                ...
                lambda_val = (
                    _mdf["lmbda"].iloc[0] if not _mdf["lmbda"].isna().all() else None
                )

                if lambda_val is not None:
                    # Check if any points from this run are visible
                    visible_points = subset[
                        (subset[f"{_prefix}/feature observed"] >= _xlim[0])
                        & (subset[f"{_prefix}/feature observed"] <= _xlim[1])
                        & (subset[f"{_prefix}/{pltcfg.key}"] >= _ylim[0])
                        & (subset[f"{_prefix}/{pltcfg.key}"] <= _ylim[1])
                    ]

                    if len(visible_points) > 0:
                        curr_range = visible_lambda_ranges[_name][dataset_name]
                        curr_range["min"] = min(curr_range["min"], lambda_val)
                        curr_range["max"] = max(curr_range["max"], lambda_val)

    return visible_lambda_ranges
...
# Track visible lambda ranges
visible_lambda_ranges = track_visible_lambda_ranges(pltcfg, metrics_dl)
...
# %%
# Get all datasets from the config
datasets = [_expcfg.subtitle for _expcfg in pltcfg.expcfgs]
# Prepare table data
table_data = []
for method in ["aco", "tafa"]:
    row = [method.upper()]

    for dataset in datasets:
        if (
            method in visible_lambda_ranges
            and visible_lambda_ranges[method] is not None
            and dataset in visible_lambda_ranges[method]
        ):
            lambda_range = visible_lambda_ranges[method][dataset]
            if lambda_range["min"] != float("inf") and lambda_range["max"] != float(
                "-inf"
            ):
                # Format as tuple (min, max)
                range_tuple = f"({lambda_range['min']:.3f}, {lambda_range['max']:.3f})"
                row.append(range_tuple)
            else:
                row.append("N/A")
        else:
            row.append("N/A")
...
print(tbl.tabulate(table_data, headers=headers, tablefmt="grid"))
