...
def _mutate_tmpls_from_prev_and_rfe_with_tracking(
    ...
    rfe_candidates: Optional[set[tuple[int, ...]]] = None,  # NEW PARAMETER
    ...
    # Add RFE candidates if provided
    if rfe_candidates is not None:
        fcs_set.update(rfe_candidates)
        for _fc in rfe_candidates:
            if _fc not in source_info:  # Don't overwrite existing entries
                source_info[_fc] = TemplateInfo(
                    source="rfe-subsets", generation=generation, parent_idx=None
                )
    # Rest of the mutation logic remains the same...
    tmpls_prv = tmpls_prv[th.sum(tmpls_prv, dim=1) - min_features > 0]
    if len(tmpls_prv) == 0:
        return fcs_set, source_info
    # mutate templates (existing random mutation logic)
    pbar = tqdm.tqdm(
...
def _update_template_candidates_rfe_with_tracking(
    ...
    new_rfe_fcs_set = _rfe_mutate_tmpls(
        tdata=tdata,
        tpcomp=tpcomp,
        tmpls_prv=tmpls_prv,
        init_fidx=init_fidx,
    ...
    # Then pass to mutate function
    fcs_set, source_info = _mutate_tmpls_from_prev_and_rfe_with_tracking(
        tmpls_prv=tmpls_prv,
        init_fidx=init_fidx,
        ...
        rfe_candidates=new_rfe_fcs_set,  # NEW PARAMETER
    )
    ...
    source_stats = {"random": 0, "previous": 0, "mutated": 0, "rfe-subsets": 0}
...
def make_templates_rfe_with_tracking(
    ...
    for _i in tqdm.trange(
        ...
        selected_sources = {"random": 0, "previous": 0, "mutated": 0, "rfe-subsets": 0}
...
output_dir: str = os.path.join("outputs", "run", data_name, "mutate-from-prev-and-rfe")
