...
import pandas as pd
import sklearn.ensemble as skl_ensmbl
    ...
    for _tidx in tqdm.tqdm(
        ...
        _newfms: th.Tensor = th.stack(
            [
                ...
                for _n_features_to_select, _estimator in itrtls.product(
                    range(min_features, int(th.sum(tmpls_prv[_tidx]).item()) + 1),
                    (
                        xgbst.XGBClassifier(),
                        skl_lm.LogisticRegression(),
                        skl_tree.DecisionTreeClassifier(),
                        skl_tree.ExtraTreeClassifier(),
                        skl_ensmbl.GradientBoostingClassifier(),
                    ),
                )
            ],
            dim=0,
...
output_dir: str = os.path.join(
    "outputs", "run", data_name, "multi-clf-rfe-candidates-selection-rate"
)
